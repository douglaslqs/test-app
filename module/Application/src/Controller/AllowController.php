...
class AllowController extends AbstractRestfulController
{
    private $allowTable;
    private $form;
    ...
    public function __construct(ResponseService $responseService, AllowTable $allowTable)
    {
    ...
    public function getAction()
    {
        ...
        if($request->isGet()){
            ...
            try {
                ...
                $allow = $this->allowTable->fetch($arrParams);
                if(!empty($allow)){
                    $this->responseService->setData($allow);
        ...
        if ($request->isPost()) {
            ...
            try {
                ...
                if ($this->form->isValid()) {
                    ...
                    $allow = $this->allowTable->fetchRow($arrParams);
                    if (empty($allow)) {
                        $returnInsert = $this->allowTable->insert($arrParams);
                        if ($returnInsert !== 1) {
                            $this->responseService->setCode(ResponseService::CODE_ERROR);
                            $this->logger->setMethodAndLine(__METHOD__, __LINE__);
                        ...
                        if (is_array($allow)) {
                            ...
                            $this->responseService->setCode(ResponseService::CODE_ERROR);
                            $this->logger->setMethodAndLine(__METHOD__, __LINE__);
                            $this->logger->save(Logger::LOG_APPLICATION,Logger::WARNING,$allow);
                        }
                    }
                } else {
            ...
            try {
                ...
                if ($this->form->isValid()) {
                    ...
                    if (is_array($allow) && !empty($allow)) {
                        ...
                        $returnUpdate = $this->allowTable->update($arrSet,$arrWhere);
