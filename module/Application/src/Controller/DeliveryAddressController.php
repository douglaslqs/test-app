...
class DeliveryAddressController extends AbstractRestfulController
{
    private $deliveryAddressTable;
    private $form;
    private $responseService;
    private $filterService;
    private $logger;

    public function __construct(ResponseService $responseService,DeliveryAddressTable $deliveryAddressTable)
    {
        $this->responseService = $responseService;
        $this->deliveryAddressTable = $deliveryAddressTable;
    }
    ...
    public function getAction()
    {
        ...
        if($request->isGet()){
            ...
            try {
                ...
                $deliveryAddress = $this->deliveryAddressTable->fetch($arrParams);
                if(!empty($deliveryAddress)){
                    $this->responseService->setData($deliveryAddress);
        ...
        if ($request->isPost()) {
            ...
            try {
                ...
                if ($this->form->isValid()) {
                    ...
                    $deliveryAddress = $this->deliveryAddressTable->fetchRow($arrParams);
                    if (empty($deliveryAddress)) {
                        $returnInsert = $this->deliveryAddressTable->insert($arrParams);
                        if ($returnInsert !== 1) {
                            $this->responseService->setCode(ResponseService::CODE_ERROR);
                            $this->logger->setMethodAndLine(__METHOD__, __LINE__);
                        ...
                        if (is_array($deliveryAddress)) {
                            ...
                            $this->responseService->setCode(ResponseService::CODE_ERROR);
                            $this->logger->setMethodAndLine(__METHOD__, __LINE__);
                            $this->logger->save(Logger::LOG_APPLICATION,Logger::WARNING,$deliveryAddress);
                        }
                    }
                } else {
            ...
            try {
                ...
                if ($this->form->isValid()) {
                    ...
                    if (is_array($deliveryAddress) && !empty($deliveryAddress)) {
                        ...
                        $returnUpdate = $this->deliveryAddressTable->update($arrSet,$arrWhere);
