...
class RoleResourceAllowController extends AbstractRestfulController
{
    ...
    public function __construct(ResponseService $responseService, RoleResourceAllowTable $roleResourceAllowTable)
    {
        $this->responseService = $responseService;
        $this->roleResourceAllowTable = $roleResourceAllowTable;
    }
    ...
    public function getAction()
    {
        ...
        if($request->isGet()){
            ...
            try {
                ...
                $roleResourceAllow = $this->roleResourceAllowTable->fetch($arrParams);
                if(!empty($roleResourceAllow)){
                    $this->responseService->setData($roleResourceAllow);
        ...
        if ($request->isPost()) {
            ...
            try {
                ...
                if ($this->form->isValid()) {
                    ...
                    $roleResourceAllow = $this->roleResourceAllowTable->fetchRow($arrParams);
                    if (empty($roleResourceAllow)) {
                        $returnInsert = $this->roleResourceAllowTable->insert($arrParams);
                        if ($returnInsert !== 1) {
                            $this->responseService->setCode(ResponseService::CODE_ERROR);
                            $this->logger->setMethodAndLine(__METHOD__, __LINE__);
                        ...
                        if (is_array($roleResourceAllow)) {
                            $this->responseService->setCode(ResponseService::CODE_ALREADY_EXISTS);
                        } else {
                            $this->responseService->setCode(ResponseService::CODE_ERROR);
                            $this->logger->setMethodAndLine(__METHOD__, __LINE__);
                            $this->logger->save(Logger::LOG_APPLICATION,Logger::WARNING,$roleResourceAllow);
                        }
                    }
                } else {
        ...
        if ($request->isPost()) {
            ...
            try {
                ...
                if ($this->form->isValid()) {
                    $arrParams = $this->filterService->setData($arrParams)->getData();
                    $roleResourceAllow = $this->roleResourceAllowTable->fetchRow($arrParams);
                    if (is_array($roleResourceAllow) && !empty($roleResourceAllow)) {
                        $arrSet = $this->filterService->getArraySet();
                        $arrWhere = $this->filterService->getArrayWhere();
                        $returnUpdate = $this->roleResourceAllowTable->update($arrSet,$arrWhere);
